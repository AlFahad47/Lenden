  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setKycStatus("pending"); // Update UI to show pending screen
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#04090f] transition-colors duration-500 px-6 py-16">
      <div className="max-w-4xl mx-auto">
        ...
        {/* CASE 3: NO KYC SUBMITTED (SHOW FORM) */}
        {!kycStatus && (
          <>
  ...
  return (
    ...
    >
      ...
      <button 
        ...
        className="mt-8 px-8 py-3 bg-gradient-to-r from-[#0095ff] to-[#0061ff] text-white rounded-xl font-semibold"
      >
        Go to Dashboard
      </button>
    </motion.div>
...
// Keep your existing Section, Input, Textarea, Select, FileInput components below...
...
function Input({ label, name, type = "text", required = false, defaultValue = "", readOnly = false }: any) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <input name={name} type={type} required={required} defaultValue={defaultValue} readOnly={readOnly} className={`bg-gray-100 dark:bg-blue-900/20 text-gray-800 dark:text-blue-100 border border-gray-300 dark:border-blue-700/40 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#0095ff] outline-none ${readOnly ? "opacity-60" : ""}`} />
    </div>
  );
}
